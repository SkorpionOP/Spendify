...
def signup(
    payload: SignupRequest, response: Response, db: PostgresWrapper = Depends(get_db)
):
    try:
        db.execute(
    ...
    response.set_cookie(
        key="session_token", value=token, httponly=True, samesite="lax", max_age=2592000
    )
    return AuthResponse(
        status="success",
        user_id=user["id"],
...
def login(
    payload: LoginRequest, response: Response, db: PostgresWrapper = Depends(get_db)
):
    user = db.execute(
