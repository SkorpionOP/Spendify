...
from backend.schemas.budget import BudgetSetup, SalaryUpdate, PercentUpdate, NeedsTopup, SavingsTopup

router = APIRouter(prefix="/budget", tags=["Budget"])

@router.post("/setup")
def setup_budget(payload: BudgetSetup, uid: int = Depends(get_current_user_id), db: PostgresWrapper = Depends(get_db)):
    ...
    db.execute(
        """INSERT OR REPLACE INTO finance
           ...
           VALUES (?, ?, ?, ?, ?, ?)""",
        (uid, payload.salary, payload.needs_percent, payload.savings_percent, needs, savings)
    )
    db.execute(
...
@router.post("/salary")
def update_salary(payload: SalaryUpdate, uid: int = Depends(get_current_user_id), db: PostgresWrapper = Depends(get_db)):
    finance = db.execute(
...
@router.post("/percent")
def update_percent(payload: PercentUpdate, uid: int = Depends(get_current_user_id), db: PostgresWrapper = Depends(get_db)):
    if payload.needs_percent + payload.savings_percent != 100:
    ...
    db.commit()
    return {"status": "success", "message": "Budget allocation percents updated successfully."}

@router.post("/topup-needs")
def topup_needs(payload: NeedsTopup, uid: int = Depends(get_current_user_id), db: PostgresWrapper = Depends(get_db)):
    finance = db.execute(
    ...
    db.commit()
    return {"status": "success", "message": f"Added ₹{payload.needs:.0f} to your budget!"}

@router.post("/topup-savings")
def topup_savings(payload: SavingsTopup, uid: int = Depends(get_current_user_id), db: PostgresWrapper = Depends(get_db)):
    finance = db.execute(
    ...
    db.execute("UPDATE monthly_budget SET savings=? WHERE user_id=?", (new_savings, uid))
    db.commit()
    return {"status": "success", "message": f"Added ₹{payload.savings:.0f} to your savings!"}
